...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, "*Terjadinya Error Sistem Agar Bisa normal Silahkan Ketik/Pencet /start Atau /🚀Start Attack Sanz Sampai Muncul Menu Format DDOS BY SANZ*", parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        args = message.text.split()
        if len(args) != 3:
            bot.send_message(message.chat.id, "*Terjadinya Error Sistem Attack Agar Normal Silahkan Ketik  /start Atau /🚀Start Attack Sanz*", parse_mode='Markdown')
            return
        target_ip, target_port, duration = args[0], int(args[1]), args[2]
...
def send_welcome(message):
    ...
    # Create buttons
    btn1 = KeyboardButton("")
    btn2 = KeyboardButton("🚀Start Attack Sanz")
    btn3 = KeyboardButton("")
...
def handle_message(message):
    ...
    elif message.text == "🚀Start Attack Sanz":
        bot.reply_to(message, "*🚀Start Attack Sanz Selected*", parse_mode='Markdown')
