...
TOKEN = '7965226544:AAE5IMb3DoN98-LdkXpy3p8kMJAnjE1qZS8'  # Replace with your actual bot token
CHANNEL_ID = '-1002362083734'  # Replace with your specific channel or group ID
# Initialize the bot
...
DAILY_ATTACK_LIMIT = 50000  # Daily attack limit per user

# List of user IDs exempted from cooldown, limits, and photo requirements
EXEMPTED_USERS = [6654576379,6654576379]
...
def reset_daily_counts():
    ...
    if ist_now >= reset_time + timedelta(days=1):
        ...
        user_photos.clear()
        user_bans.clear()
        reset_time = ist_now.replace(hour=0, minute=3, second ok=0, microsecond=0) + timedelta(days=1)
...
def bgmi_command(message):
    ...
    if user_id not in EXEMPTED_USERS:
        ...
        if user_id in user_cooldowns:
            ...
            if datetime.now() < cooldown_time:
                ...
                bot.send_message(
                    message.chat.id,
                    f"⚠️⚠️ 𝙃𝙞 {message.from_user.first_name}, 𝙮𝙤𝙪 𝙖𝙧𝙚 𝙘𝙪𝙧𝙧𝙚𝙣𝙩𝙡𝙮 𝙤𝙣 𝙘𝙤𝙤𝙡𝙙𝙤𝙬𝙣. 𝙋𝙡𝙚𝙖𝙨𝙚 𝙬𝙖𝙞𝙩 {remaining_time // 5} 𝙢𝙞𝙣𝙪𝙩𝙚𝙨 𝙖𝙣𝙙 {remaining_time % 5} 𝙨𝙚𝙘𝙤𝙣𝙙𝙨 𝙗𝙚𝙛𝙤𝙧𝙚 𝙩𝙧𝙮𝙞𝙣𝙜 𝙖𝙜𝙖𝙞𝙣 ⚠️⚠️ "
                )
                return
        ...
        if user_id in user_attacks and user_attacks[user_id] > 0 and not user_photos.get(user_id, False):
            ...
            bot.send_message(
                message.chat.id,
                f"𝙃𝙞 {message.from_user.first_name}, ⚠️⚠️𝙔𝙤𝙪 𝙝𝙖𝙫𝙚𝙣'𝙩 𝙥𝙧𝙤𝙫𝙞𝙙𝙚𝙙 𝙛𝙚𝙚𝙙𝙗𝙖𝙘𝙠 𝙖𝙛𝙩𝙚𝙧 𝙮𝙤𝙪𝙧 𝙡𝙖𝙨𝙩 𝙖𝙩𝙩𝙖𝙘𝙠. 𝙔𝙤𝙪 𝙖𝙧𝙚 𝙗𝙖𝙣𝙣𝙚𝙙 𝙛𝙧𝙤𝙢 𝙪𝙨𝙞𝙣𝙜 𝙩𝙝𝙞𝙨 𝙘𝙤𝙢𝙢𝙖𝙣𝙙 𝙛𝙤𝙧 3 𝙢𝙞𝙣𝙪𝙩𝙚𝙨 ⚠️⚠️"
            )
            return
    ...
    try:
        ...
        logging.info(f"Attack started by {user_name}: ./ALONE {target_ip} {target_port} {default_duration} 677")
